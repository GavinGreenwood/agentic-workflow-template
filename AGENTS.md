...
**The fresh-context property is Claude Code's, not the contract's.** Codex propagates context with `fork_turns`: omitted or `"all"` means the spawned role sees the parent's whole conversation, which is the opposite of the fresh, independently-grounded read the role exists to give. A Codex delegation that wants the role's actual behaviour has to ask for it — `fork_turns: "none"`, or a small number of turns. **The role's pinned model does bind on Codex, as last measured** — measured, not inferred, on Codex 0.148.0: a `consultant` spawned from a `gpt-6-luna` parent ran its own turns on the role's pinned `gpt-6-sol`, both with `fork_turns` omitted and with `fork_turns: "none"` passed explicitly; the child's session rollout records a model switch to the pin before its first turn. The same test on the 5.6 models, on the same Codex version, left the child on the parent's model, as Codex documents for a full-history fork (it inherits the parent's model and _does not accept overrides_) — so the behaviour has flipped once already. `verify:agents` asserts the pin is written down, which is not the same as it taking effect: if the model matters, check the child's rollout or set it on the session. The role's `sandbox_mode` is still instruction-level only on Codex.
