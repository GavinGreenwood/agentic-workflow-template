...
If the ticket input is a full Jira URL (e.g. `https://*.atlassian.net/browse/$JIRA_PROJECT_KEY-42`), extract the ticket key from the URL path. If it is a bare number (e.g. `42`), prepend `$JIRA_PROJECT_KEY-`. Store the result as `TICKET_ID` and use it for all curl commands below.
...
1. For each fix in the list:
   ...
   - Apply the change.
   - If the fix requires a new or updated test, write it following TDD conventions (see AGENTS.md).
