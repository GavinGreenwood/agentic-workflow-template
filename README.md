...
- The advisor must be **at least as capable as the main model**. `claude-opus-5-5` covers Haiku, Sonnet, and supported Opus mains; a Fable 5 main accepts only `fable`. An unsatisfiable pairing is rejected outright rather than downgraded, so the advisor just does not attach — no fixed value is portable across every possible main model.
